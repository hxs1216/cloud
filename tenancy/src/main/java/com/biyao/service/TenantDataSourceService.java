...
public class TenantDataSourceService {
    ...
    public void addDataSource(DataSourceInfo dataSourceInfo) {
        ...
        if (Constants.PROFILE_MYSQL.equalsIgnoreCase(dataSourceInfo.getType())) {
            this.initDatabase(dataSourceInfo);
    ...
    public Boolean checkAllDataSource(String tenantCode) {
        return TenantDataSourceProvider.checkTenantDataSourceExist(tenantCode) || TenantMongoDatabaseProvider.checkDataBaseExist(tenantCode);
    }
    ...
    /**
     * 初始化本地配置的数据库（已初始化本地数据源  则不会初始化远程数据源）
     */
